...
testthat::test_that("basic output df has proper dimensions (mobile)", {
  ...
  y <- pagespeedParseR:::lh_simple_2_vec("https://www.google.com", strategy = "mobile", interval = 0, long_result = F)
  testthat::expect_equal(ncol(x), 2)
  testthat::expect_equal(nrow(y), 1)
})
...
testthat::test_that("basic output df has proper dimensions (both devices)", {
  ...
  y <- pagespeedParseR:::lh_simple_2_vec("https://www.google.com", strategy = c("mobile", "desktop"), interval = 0, long_result = F)
...
testthat::test_that("output df returns proper dimensions despite the NULL", {
  ...
  x_l <- pagespeedParseR:::lh_simple_2_vec(url = c(NULL, "https://www.google.com"), strategy = "desktop", interval = 0, long_result = F)
  y <- pagespeedParseR:::lh_simple_2_vec(url = c("https://www.google.com", NULL), strategy = "mobile", interval = 0)
  y_l <- pagespeedParseR:::lh_simple_2_vec(url = c("https://www.google.com", NULL), strategy = "mobile", interval = 0, long_result = F)

  testthat::expect_equal(length(dim(x)), 2)
  testthat::expect_equal(length(dim(x_l)), 2)
  testthat::expect_equal(length(dim(y)), 2)
  testthat::expect_equal(length(dim(y_l)), 2)

  testthat::expect_equal(ncol(x), 2)
  testthat::expect_equal(nrow(x_l), 1)
  testthat::expect_equal(ncol(y), 2)
  testthat::expect_equal(nrow(y_l), 1)
})

testthat::test_that("output df returns rows/columns with errors", {
  x <- pagespeedParseR:::lh_simple_2_vec(url = c("loremipsumdolorametpageparsererrortest.com", "https://www.google.com"), strategy = "desktop", interval = 0)
  y <- pagespeedParseR:::lh_simple_2_vec(url = c("loremipsumdolorametpageparsererrortest.com", "https://www.google.com"), strategy = "desktop", interval = 0, long_result = F)
  testthat::expect_equal(ncol(x), 3)
  testthat::expect_equal(nrow(y), 2)
})
